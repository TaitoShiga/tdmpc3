#!/usr/bin/env python3
"""Hopper Hop Backwards 4モデル訓練スクリプト

Baseline, DR, Model C, Oracleの4モデルを学習する。
各モデルを5 seeds (0-4) で訓練。

物理パラメータ: thigh_length (大腿長)
- デフォルト: 0.33m
- DR範囲: [0.25, 0.45] (0.76×～1.36×)
"""
...
MODELS = [
    {
        'name': 'Baseline',
        'task': 'hopper-hop_backwards',  # 固定長 (0.33m)
    ...
    {
        'name': 'DR',
        'task': 'hopper-hop_backwards_randomized',  # ランダム長 (0.25-0.45)
        'exp_name': 'hopper_dr',
        'use_oracle': False,
        'use_model_c': False,
        'steps': 1000000,
...
def main():
    ...
    print("="*70)
    print("Hopper Hop Backwards 4モデル連続学習開始")
    ...
    print(f"\n物理パラメータ: thigh_length")
    print(f"  - デフォルト: 0.33m")
    print(f"  - DR範囲: [0.25, 0.45]")
    print()
